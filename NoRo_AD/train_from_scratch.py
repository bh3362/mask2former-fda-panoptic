...
# # Mask2Former Training Script for CARLA COCO-panoptic subset
# # - 로더(resister.py)에서 19 trainIds로 이미 치환
# # - NUM_CLASSES = 19 고정
# # - 학습: LSJ 매퍼 / 평가: 안전 매퍼
# # - 카테고리/매핑 강제 검사 + 평가 시 실제 GT JSON 경로/카테고리 개수 출력
# # - ▶추가: 평가 시 panoptic 예측을 항상 OUTPUT_DIR/inference_<dataset>/panoptic_pred 에 저장(+경로 로그)
...
# import resister  # 경로/데이터셋 등록 (이미 19클래스 매핑 세팅)
...
#     COCOEvaluator,
#     COCOPanopticEvaluator,     # 기본 evaluator
#     DatasetEvaluators,
#     LVISEvaluator,
#     SemSegEvaluator,
#     verify_results,
...
#     # [수정 1] resister.py에서 등록한 실제 데이터셋 이름으로 변경
#     TRAIN = "carla_final_panoptic_train"
#     VAL   = "carla_final_panoptic_val"
...
#     # 전처리 데이터셋은 19개 클래스(0-18)와 255(ignore)를 사용합니다.
#     cfg.MODEL.SEM_SEG_HEAD.NUM_CLASSES = 19
#     cfg.MODEL.SEM_SEG_HEAD.IGNORE_VALUE = 255
#     cfg.INPUT.MASK_FORMAT = "bitmask"

#     # 출력 경로 (하드코딩된 경로를 사용하거나, --opts OUTPUT_DIR ... 로 덮어쓰기 가능)
...
#     # --- [수정 2] 메타 최종 확인: resister.py의 항등 매핑(0-18 -> 0-18) 검증 ---
#     print("\n[META] final_dataset 메타데이터 검증 시작...")
...
#         # (resister.py의 thing_classes_final, stuff_classes_final 기준)
...
# train_from_scratch_FINAL.py
# [!!!] resister.py의 등록 코드가 setup() 함수 내부로 통합됨

# train_from_scratch.py
# [!!!] resister.py의 등록 코드가 setup() 함수 내부로 통합됨
# [!!!] FDA(주파수 도메인 적응) 코드는 포함되지 않음 (베이스라인)
...
def register_carla_panoptic(cfg):
    ...
    """
    # [!!!] 'road' 버그가 해결된 'final_dataset3'을 가리킵니다.
    ROOT = "/media/vip-dell/HC/final_dataset3" 
