...
# Override via the CARLA_DATASET_ROOT env var; this default documents the
# path actually used for the thesis run.
CARLA_DATASET_ROOT = os.environ.get("CARLA_DATASET_ROOT", "/media/vip-dell/HC/final_dataset5")

from tqdm import tqdm
import numpy as np
import cv2  # used by SavePanopticPredictions below
...
def setup(args):
    ...
    # Baseline (Non-FDA) run: register the raw CARLA images.
    TRAIN, VAL = register_carla_panoptic(
        CARLA_DATASET_ROOT, "leftImg8bit", "carla_final_panoptic_train", "carla_final_panoptic_val"
    )
    ...
    if not cfg.OUTPUT_DIR:
        cfg.OUTPUT_DIR = os.environ.get("OUTPUT_DIR", "./output/train_baseline")
    ...
    print(f"\n[META] {CARLA_DATASET_ROOT} metadata verification starting...")
