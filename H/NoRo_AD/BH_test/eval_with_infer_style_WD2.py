...
## CARLA로 학습된 Cityscapes-19 모델 + WD2 이미지용 최종 코드
...
# --- 프로젝트 경로 & CARLA 데이터셋 등록 (그대로 유지) ---
sys.path.insert(0, "/home/vip-dell/NoRo_AD")
import resister  # carla_final_panoptic_{train,val} 등록용 (메타 사용)
...
# --- Cityscapes trainId 기준 ID 매핑 (학습/평가랑 동일) ---
...
def build_cfg(args):
    ...
    # VisualizationDemo가 metadata를 끌어올 이름
...
def run_on_image(predictor, img_bgr, meta):
    """
    WD2 / Cityscapes / CARLA 상관없이
    - 입력: 임의 이미지 (BGR)
    - 출력: overlay, panoptic id map, segments_info
    """
    ...
    # ---- Visualizer용 category_id 재매핑 ----
    mapped_segments = []
    for seg in segments_info:
        seg_new = seg.copy()
        cid = seg_new["category_id"]
        if seg_new.get("isthing", False):
...
def main():
    ...
    parser.add_argument("--input", required=True, nargs="+",
                        help="이미지 파일/디렉토리/글롭 패턴 (예: wd_public_v2p0/images/*.jpg)")
    ...
    # detectron2/demo 의 VisualizationDemo 사용
    from demo.predictor import VisualizationDemo
    demo = VisualizationDemo(cfg)
    predictor = demo.predictor

    # CARLA final 메타 (Cityscapes-19와 호환)
