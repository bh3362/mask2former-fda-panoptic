...
#from mmseg.utils import get_root_logger -> 기존 코드

##추가 {
#from mmengine.logging import get_logger as get_root_logger 
#from mmengine import MMLogger 
#get_logger = MMLogger.get_instance

from mmengine.logging import MMLogger

def get_root_logger(name='mask2former', log_level='INFO'):
    return MMLogger.get_instance(name, log_level=log_level)
...
class ConvPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=384, patch_size=16, in_chans=3, embed_dim=768, act_layer=nn.GELU):
        super().__init__()
...
class HybridEmbed(nn.Module):
    ...
    """
    def __init__(self, backbone, img_size=384, patch_size=2, feature_size=None, in_chans=3, embed_dim=384):
        super().__init__()
...
class D2FANTransformer(Backbone):
    def __init__(self, cfg, input_shape):
        super().__init__()

        # FAN backbone 모델 설정 (체크포인트 로드 생략 가능)
        self.fan_model = fan_large_16_p4_hybrid(
            patch_size=cfg.MODEL.FAN.PATCH_SIZE,
            in_chans=cfg.MODEL.FAN.IN_CHANS,
            num_classes=cfg.MODEL.FAN.NUM_CLASSES,
            embed_dim=cfg.MODEL.FAN.EMBED_DIM,
            depth=cfg.MODEL.FAN.DEPTH,
            backbone=None,
            out_idx=cfg.MODEL.FAN.OUT_IDX,
            num_heads=cfg.MODEL.FAN.NUM_HEADS,
            mlp_ratio=cfg.MODEL.FAN.MLP_RATIO,
            qkv_bias=True,
            drop_rate=cfg.MODEL.FAN.DROP_RATE,
            attn_drop_rate=cfg.MODEL.FAN.ATTN_DROP_RATE,
            drop_path_rate=cfg.MODEL.FAN.DROP_PATH_RATE,
            act_layer=None,
            norm_layer=None,
            cls_attn_layers=cfg.MODEL.FAN.CLS_ATTN_LAYERS,
            use_pos_embed=True,
            eta=cfg.MODEL.FAN.ETA,
            tokens_norm=True,
        ...
        self._out_features = cfg.MODEL.FAN.OUT_FEATURES

    def forward(self, x):
        outputs = {}
        fan_features = self.fan_model(x)
        for i, layer_name in enumerate(self._out_features):
            outputs[layer_name] = fan_features[i]
        # print("FAN outputs", outputs.items())
        return outputs
...
# # FAN 모델 인스턴스화 (fan_large_16_p4_hybrid 사용)
# fan_model = fan_large_16_p4_hybrid(
#     patch_size=16, in_chans=3, num_classes=19, embed_dim=480, depth=22, 
#     backbone=None, out_idx=18, num_heads=10, mlp_ratio=4., qkv_bias=True, 
#     drop_rate=0., attn_drop_rate=0., drop_path_rate=0., act_layer=None, 
#     norm_layer=None, cls_attn_layers=2, use_pos_embed=True, eta=1., 
#     tokens_norm=True, use_checkpoint=False
# )
# print(fan_model)
# checkpoint = torch.load('/home/ailab/Desktop/hyungchan/Mask2Former/mask2former/modeling/backbone/fan_hybrid_large_in22k_1k_384.pth.tar')

# model_dict = fan_model.state_dict()
# # 체크포인트의 키를 모델의 키와 맞추는 과정
# pretrained_dict = {k: v for k, v in checkpoint.items() if k in model_dict}

# # 업데이트된 가중치 적용
# model_dict.update(pretrained_dict)
# fan_model.load_state_dict(model_dict)

# # 현재 모델의 가중치와 체크포인트의 가중치가 일치하는지 확인
# missing_keys, unexpected_keys = fan_model.load_state_dict(checkpoint, strict=False)
...
# # FAN 모델의 최종 출력도 확인
# fan_output = fan_model(input_tensor)
# print(f"fan_output: {len(fan_output)} outputs")

# # fan_output의 각 요소에 대한 shape 출력
# for i, output in enumerate(fan_output):
#     print(f"fan_output [{i}].shape = {output.shape}")





# fpn_outputs = fpn_model(fan_features)


# # FAN과 FPN을 결합한 모델 인스턴스화
# fan_fpn_model = FAN_FPN(fan=fan_model, fpn=fpn_model)

# # 예시 입력
# input_tensor = torch.randn(1, 3, 224, 224)

# # 모델의 출력
# outputs = fan_fpn_model(input_tensor)
